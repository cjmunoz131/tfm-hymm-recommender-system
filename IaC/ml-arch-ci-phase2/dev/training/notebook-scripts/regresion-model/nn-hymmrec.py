...
class MultimodalExplainableGMF(nn.Module):
    def __init__(self, num_users, num_items, num_categories, emb_dim=64, aws_dim=1024):
        ...
        # GMF Layer (Caja amarilla del diagrama)
        # Recibe el producto de ambos vectores (emb_dim) y lo reduce a 1 predicción
        self.gmf_layer = nn.Linear(emb_dim, 1)

    def forward(self, user_id, item_id, cat_id, text_emb, img_emb):
        # 1. Obtener vectores de ambas torres
        mf_user_vector = self.user_tower(user_id)
        mf_item_vector, attention_weights = self.item_tower(item_id, cat_id, text_emb, img_emb)

        # 2. Interacción GMF (Símbolo ⨂ - Producto elemento a elemento)
        interaction = mf_user_vector * mf_item_vector

        # 3. Explicabilidad y Predicción
        rating = self.gmf_layer(interaction)

        # NOTA SOBRE EL SIGMOID:
        # Si predices ratings de 1 a 5, un Sigmoid puro te dará valores entre 0 y 1.
        # Es mejor escalar el sigmoid: torch.sigmoid(rating) * 4.0 + 1.0
        # O simplemente devolver el 'rating' crudo y usar MSELoss.
        return torch.sigmoid(rating), attention_weights #* 4.0 + 1.0 # Escala para ratings de 1 a 5